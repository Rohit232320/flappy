...
void gnd_update(float dt) {
  ...
  if (gnd_tx.id != 0 && x_offset <= -gnd_tx.width)
    x_offset += gnd_tx.width;
}

void bg_draw(void) {
  if (bg_tx[curr_bg_tx].id != 0) {
      for (float x = 0; x < GAME_WIDTH; x += bg_tx[curr_bg_tx].width) {
          DrawTexture(bg_tx[curr_bg_tx], (int)x, 0, WHITE);
      }
  }
}
...
void gnd_draw(void) {
  if (gnd_tx.id != 0) {
      for (float x = x_offset; x < GAME_WIDTH; x += gnd_tx.width) {
          DrawTexture(gnd_tx, (int)x, GROUND_Y, WHITE);
      }
  }
}
